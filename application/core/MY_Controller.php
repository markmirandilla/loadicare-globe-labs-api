...
class MY_Controller extends CI_Controller
{
	...
	*/
	...
	/**
	 * Checks for valid signature hash
	 *
	 * @throws Exception when detected as an invalid signature
	 */
	public function validate_signature()
	{
		...
		$is_test 	= $this->$http_method('is_test');
		$signature 	= $this->$http_method('signature');
		if($is_test && ENVIRONMENT === 'development')
		{
			//do not validate hash
		} else
		{
			//validate signature hash
			$params = $this->input->$http_method();
			validate_hash($params,$signature);	
		}
	}
	...
	public function response($data = array(), $http_code = null)
	{
		...
		else
		{
			...
			{
					$data['status'] = isset($data['status']) ? $data['status'] : API_STATUS_ERROR;
					if(isset($data['message'])) log_message('ERROR',$data['message']);
			}

			if($http_code === 200) 
			{
				$data['message'] = isset($data['message']) ? $data['message'] : 'Success';
				$data['status'] = isset($data['status']) ? $data['status'] : API_STATUS_OK;
	...
	public function canonical_signup($email,$username,$password,$latitude,$longitude,
		...
		$fname = NULL,$lname = NULL,$avatar = NULL,$gender = NULL,$birthday = NULL,$facebook_id = NULL) 
	{
		...
		log_message('DEBUG',__METHOD__ . ' creating new user params '.print_r($params,true));
		$user_node = $this->user_model->set_as_public(TRUE)->create_node($params);

		//asynchronously extract image from url and save to s3
		/*$method_params = array('user',$user_node['id'],$avatar);
		exec_background_process('Image_processor','save_image_url_to_s3','library',$method_params);*/
		
		//associate all the categories to a user
		/* @deprecated [mark] we now filter node categories in feeds
		$user_id = $user_node['id'];
		$data = array();
		$this->load->model('category_model');
		$categories = $this->category_model->get_all_nodes(NULL,NULL);
		$insert_data = array();
		...
		$this->category_model->batch_create_node($insert_data,TABLE_USER_CATEGORIES);
		*/
	 ...
	 * @throws Exception error when the required parameters have empty/null value
	 **/
	public function set_required_fields($required_fields = array())
	{
		...
		if(array_has_value($required_fields))
		{
			foreach($required_fields as $value)
			{
				$tmp_val = $this->$http_method($value);
				if(!has_value($tmp_val)) throw new Exception("Parameter {$value} is missing");
			}
		}
		benchmark_end(__METHOD__);
	}
	...
	/**
	 * converts the equivalent user_id of the access_token
	 *
	 * @param 	(string) 	$access_token
	 * @throws Exception error when the there is no matched user_id for the access_token
	 **/
	public function get_user_id_from_access_token()
	{
		// Try and get the access token via an access_token or oauth_token parameter
		switch ($this->input->server('REQUEST_METHOD'))
		{
			...
			case 'DELETE':
				$access_token = $this->delete('access_token');
				break;
		}
		$user_id = $this->user_model->get_user_id_by_access_token($access_token);
		if(!has_value($user_id)) throw new Exception("Invalid access token");
		return $user_id;
	}
